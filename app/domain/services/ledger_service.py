...
class LedgerService:
    ...
    async def create_transfer_entries(
        ...
        source_balance = await self._lock_and_validate_balance(
            wallet_id=source_wallet_id, required_amount=amount, currency=currency
        )
    ...
    async def create_credit_entry(
        ...
        await self._update_cached_balance(
            wallet_id=wallet_id, amount=amount, entry=entry
        )
    ...
    async def create_debit_entry(
        ...
        balance = await self._lock_and_validate_balance(
            wallet_id=wallet_id, required_amount=amount, currency=currency
        )
        ...
        query = select(
            func.coalesce(
                func.sum(
                    func.case(
                        (
                            LedgerEntryORM.entry_type == EntryType.CREDIT,
                            LedgerEntryORM.amount,
                        ),
                        else_=Decimal("0"),
                    )
                ),
            ...
            func.coalesce(
                func.sum(
                    func.case(
                        (
                            LedgerEntryORM.entry_type == EntryType.DEBIT,
                            LedgerEntryORM.amount,
                        ),
                        else_=Decimal("0"),
                    )
    ...
    async def reconcile_wallet(self, wallet_id: UUID) -> dict[str, Any]:
        """
    ...
    async def _get_balance(self, wallet_id: UUID) -> WalletBalanceORM:
        ...
        query = select(WalletBalanceORM).where(WalletBalanceORM.wallet_id == wallet_id)
        result = await self.session.execute(query)
    ...
    async def _lock_and_validate_balance(
        self, wallet_id: UUID, required_amount: Decimal, currency: CurrencyCode
    ) -> WalletBalanceORM:
        """
    ...
    async def _update_cached_balance(
        self, wallet_id: UUID, amount: Decimal, entry: LedgerEntryORM
    ) -> None:
        """
